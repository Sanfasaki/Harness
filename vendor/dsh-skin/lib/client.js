  ...
  factory: (require) => {
    ...
    function defaultCfg() {
      return {
        ...
        autoSwitch: { enabled: false, intervalMin: 10 },
        presets: []
      };
    }
    ...
    function normalizeCfg(c) {
      ...
      return {
        ...
        autoSwitch: (c.autoSwitch && typeof c.autoSwitch === "object") ? (function () {
          var iv = Number(c.autoSwitch.intervalMin);
          if (!isFinite(iv) || iv <= 0) iv = d.autoSwitch.intervalMin;
          if (iv < 0.1) iv = 0.1;
          if (iv > 1440) iv = 1440;
          return { enabled: c.autoSwitch.enabled === true, intervalMin: iv };
        })() : d.autoSwitch
      };
    }
    ...
    function apply(ctx) {
      ...
      // 遮罩 = z-index:-1 的不透明底色层（盖住壁纸、不影响 UI、不拦截交互）；
      // 动画用 Web Animations API（element.animate）驱动 opacity —— JS 直接控制，
      // 完全不经过 CSS transition 级联（颜色过渡类的 !important 覆盖不到），
      // 也不依赖 cross-fade/@property/keyframes（渐变作 cross-fade 参数实测无效）。
      var transitionTimer = null;
      var activeCleanup = null;
      function sameImg(a, b) {
        if (!a && !b) return true;
        if (!a || !b) return false;
        return a.url === b.url && (a.opacity || 1) === (b.opacity || 1);
      }
      // 当前 live 皮肤与哪个预设一致（accent/text/各图片槽位全等）；无匹配返回 -1
      function matchLivePreset(list, c) {
        for (var i = 0; i < list.length; i++) {
          var s = list[i].skin || {};
          if ((s.accent || "") !== (c.accent || "")) continue;
          if ((s.text || "") !== (c.text || "")) continue;
          var si = s.images || {};
          var ci = c.images || {};
          if (sameImg(si.wallpaper, ci.wallpaper) && sameImg(si.sidebar, ci.sidebar)
            && sameImg(si.chat, ci.chat) && sameImg(si.input, ci.input) && sameImg(si.bubble, ci.bubble)) return i;
        }
        return -1;
      }
      function withThemeTransition(oldWp, newWp, applyFn) {
        ...
        var canAnim = oldUrl && newUrl && oldUrl !== newUrl && !newIsVideo;
        var veil = null;
        var finished = false;
        function cleanup() {
          if (finished) return;
        ...
        if (canAnim) {
          ...
          function run(fadeOut) {
            if (finished) return;
            veil.animate(
              [{ opacity: fadeOut ? 0 : 1 }, { opacity: fadeOut ? 1 : 0 }],
              { duration: fadeOut ? T_OUT : T_IN, easing: "linear", fill: "forwards" }
            );
          }
          // 新壁纸预解码（大图解码异步），然后启动三段式
          var img = new Image();
          var started = false;
          function start() {
            if (started) return;
            ...
            setTimeout(function () {
              ...
              setTimeout(function () {
                if (finished) return;
                run(false); // 阶段3：遮罩揭开，新壁纸渐渐出现
                if (transitionTimer) clearTimeout(transitionTimer);
                transitionTimer = setTimeout(cleanup, T_IN + 150);
              }, T_VOID);
            }, T_OUT);
          }
          img.onload = start;
          img.onerror = start; // 解码失败也照常启动，至少不阻塞切换
          img.src = newUrl;
      ...
      function persist(cfgObj) {
        ...
        saveTimer = setTimeout(function () {
          var json = JSON.stringify(pendingCfg);
          saveChain = saveChain.then(function () {
            return fetch("/api/dsh-skin-state", {
              method: "POST",
      ...
      function Control() {
        ...
        cfgRef.current = cfg;
        var activePresetIdx = matchLivePreset(cfg.presets, cfg);
        ...
        // 自动切换主题定时器：开启且预设≥2 时按间隔循环应用预设列表。
        // 循环方向永远从"当前生效皮肤"在列表中的位置前进一格（到底回卷）：
        // 与整数索引解耦，手动应用/刷新/皮肤微调都不会造成跳变或折返。
        React.useEffect(function () {
          var as = cfg.autoSwitch;
          if (!as || !as.enabled) return;
          if (!cfg.presets || cfg.presets.length < 2) return;
          var ms = Math.round(as.intervalMin * 60000);
          var timer = setInterval(function () {
            var cur = cfgRef.current;
            var list = cur.presets;
            if (!list || list.length < 2) return;
            var idx = matchLivePreset(list, cur);
            ...
            withThemeTransition(oldWp, newWp, function () {
              setCfg(function (c) {
        ...
        }, [cfg.autoSwitch && cfg.autoSwitch.enabled, cfg.autoSwitch && cfg.autoSwitch.intervalMin, cfg.presets.length]);
        ...
        React.useEffect(function () {
          ...
          if (loaded) {
            if (autoApplyRef.current) {
              // 自动切换应用的主题只是临时的实时状态，不落盘（预设才是持久化来源）
        ...
        function pickScheme(id) {
          withThemeTransition(null, null, function () {
            setScheme(id);
        ...
        function applyPreset(name) {
          ...
          var oldWp = (cur.images && cur.images.wallpaper) ? cur.images.wallpaper : null;
          var newWp = null;
          for (var i = 0; i < cur.presets.length; i++) {
            if (cur.presets[i].name === name && cur.presets[i].skin && cur.presets[i].skin.images) {
              newWp = cur.presets[i].skin.images.wallpaper;
              break;
            }
          }
          withThemeTransition(oldWp, newWp, function () {
            setCfg(function (c) {
              for (var j = 0; j < c.presets.length; j++) {
                if (c.presets[j].name === name) {
                  var s = c.presets[j].skin || {};
                  return Object.assign({}, c, {
                    enabled: true,
                    accent: s.accent || c.accent,
                    text: s.text || c.text,
        ...
        function toggleAutoSwitch() {
          setCfg(function (c) {
            var as = Object.assign({}, c.autoSwitch, { enabled: !c.autoSwitch.enabled });
            return Object.assign({}, c, { autoSwitch: as });
          });
        }
        function setAutoIntervalMin(v) {
          var n = Number(v);
          if (!isFinite(n) || n <= 0) return;
          if (n < 0.1) n = 0.1;
          if (n > 1440) n = 1440;
          setCfg(function (c) {
            var as = Object.assign({}, c.autoSwitch, { intervalMin: n });
            return Object.assign({}, c, { autoSwitch: as });
          });
        }
          ...
          open ? h("div", { className: "dsh-skin-panel" },
            ...
            h("div", { className: "dsh-skin-body" },
              ...
              h("div", { className: "dsh-skin-section" },
                ...
                cfg.presets.map(function (p, pi) {
                  return h("div", { className: "dsh-skin-preset-row" + (pi === activePresetIdx ? " dsh-skin-preset-active" : ""), key: p.name },
                    h("span", { className: "dsh-skin-preset-name" }, p.name),
                    pi === activePresetIdx ? h("span", { className: "dsh-skin-hint" }, "当前") : null,
              ...
              h("div", { className: "dsh-skin-section" },
                h("div", { className: "dsh-skin-label" }, "自动切换主题"),
                h("div", { className: "dsh-skin-row" },
                  h("button", {
                    className: "dsh-skin-chip" + (cfg.autoSwitch.enabled ? " dsh-skin-chip-on" : ""),
                    onClick: toggleAutoSwitch,
                    disabled: cfg.presets.length < 2
                  }, cfg.autoSwitch.enabled ? "开" : "关"),
                  h("span", { className: "dsh-skin-hint" }, cfg.presets.length < 2 ? "需要至少 2 个预设" : "按列表顺序循环切换")
                ),
                h("div", { className: "dsh-skin-row" },
                  h("button", { className: "dsh-skin-chip" + (cfg.autoSwitch.intervalMin === 1 ? " dsh-skin-chip-on" : ""), onClick: function () { setAutoIntervalMin(1); } }, "1min"),
                  h("button", { className: "dsh-skin-chip" + (cfg.autoSwitch.intervalMin === 10 ? " dsh-skin-chip-on" : ""), onClick: function () { setAutoIntervalMin(10); } }, "10min"),
                  h("button", { className: "dsh-skin-chip" + (cfg.autoSwitch.intervalMin === 30 ? " dsh-skin-chip-on" : ""), onClick: function () { setAutoIntervalMin(30); } }, "30min"),
                  h("button", { className: "dsh-skin-chip" + (cfg.autoSwitch.intervalMin === 60 ? " dsh-skin-chip-on" : ""), onClick: function () { setAutoIntervalMin(60); } }, "1h")
                ),
                h("div", { className: "dsh-skin-row" },
                  h("span", { className: "dsh-skin-hint" }, "自定义"),
                  h("input", { className: "dsh-skin-num", type: "number", min: 0.1, max: 1440, step: 0.5, value: cfg.autoSwitch.intervalMin, onChange: function (e) { setAutoIntervalMin(e.target.value); } }),
                  h("span", { className: "dsh-skin-hint" }, "分钟")
                )
              ),
